...
PixelRGBA8 Blend::composite(const PixelRGBA8& dst,
    ...
    BlendMode mode)
{
    ...
    // Учитываем внешнюю непрозрачность (слой/инструмент)
    ...
    // Сначала вычисляем blended color (B(d,s)) по каналам
    float br = blendChannel(mode, dr, sr);
    float bg = blendChannel(mode, dg, sg);
    float bb = blendChannel(mode, db, sb);

    // Теперь делаем SRC_OVER, но вместо srcColor используем blendedColor.
    // outA = sa + da*(1-sa)
    float outA = sa + da * (1.0f - sa);

    // Для корректной формулы используем premultiplied промежуточно:
    // outRGB_premul = blendedRGB * sa + dstRGB * da * (1-sa)
    float outR_p = br * sa + dr * da * (1.0f - sa);
    float outG_p = bg * sa + dg * da * (1.0f - sa);
    float outB_p = bb * sa + db * da * (1.0f - sa);

    // Разделяем обратно на outA (unpremultiply)
...
PixelRGBA8 Blend::apply(BlendMode mode,
