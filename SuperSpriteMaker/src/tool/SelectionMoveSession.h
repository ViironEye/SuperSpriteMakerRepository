...
public:
    ...
    bool begin(Frame* frame, const SelectionMask& sel, MoveMode mode) {
        ...
        m_cutDiffs.clear();
        if (m_mode == MoveMode::Cut) m_cutDiffs.reserve(size_t(bw) * size_t(bh));
    ...
    void renderPreviewTo(PixelBuffer& dst) const 
    {
        ...
        dst.blitFrom(m_floating, 0, 0, m_floating.width(), m_floating.height(), m_boundsX + m_offsetX, m_boundsY + m_offsetY);
    }
    ...
    std::unique_ptr<Command> commit() 
    {
        ...
        for (const auto& d : m_cutDiffs) diffs.push_back(d);
        ...
        for (int y = 0; y < m_boundsH; ++y) 
        {
            for (int x = 0; x < m_boundsW; ++x) 
            {
                ...
                PixelRGBA8 after = srcP;

                if (before.r == after.r && before.g == after.g && before.b == after.b && before.a == after.a) continue;
