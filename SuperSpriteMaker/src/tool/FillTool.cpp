...
{
    // tolerance = max per-channel difference
    ...
    if (s.includeAlpha)
        if (iabs((int)a.a - (int)b.a) > s.tolerance) return false;
...
void FillTool::apply(Frame* frame, StrokeCommand* cmd, int x, int y, float /*pressure*/)
{
    ...
    // В текущей версии работаем с RGBA8
    ...
    // Если заливаем тем же цветом — выходим
