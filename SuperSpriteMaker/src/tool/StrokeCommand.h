...
public:
    ...
    void recordPixel(int x, int y, const PixelRGBA8& before, const PixelRGBA8& after)
    {
        ...
        if (slot == -1) {
            ...
            // before не трогаем Ч он должен отражать состо€ние ƒќ первого изменени€ пиксел€
...
private:
    ...
    std::vector<int> m_indexMap;     // (y*w+x) -> индекс в m_diffs или -1
